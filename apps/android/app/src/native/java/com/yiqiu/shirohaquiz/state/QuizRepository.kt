...
object QuizRepository {
    ...
    val studyProgress = mutableStateMapOf<String, SectionProgress>()
    ...
    fun init(context: Context) {
        ...
        knowledgeCourses.addAll(restoredKnowledgeCourses)
        studyProgress.putAll(restoredStudyProgress)
    ...
    private fun evaluateQuestion(question: Question, userAnswer: List<String>): QuestionCheckResult {
        ...
        val correct = judgeQuestionCore(question, normalizedUserAnswer)
    ...
    /**
     * 序列化知识课程列表（嵌套对象列表范式）
     * 每个课程包含课程元信息 + 章节列表（章节包含嵌套字段）
     */
    private fun knowledgeCoursesToJson(courses: List<KnowledgeCourse>): String {
        val array = JSONArray()
        courses.forEach { course ->
            val item = JSONObject()
            item.put("courseId", course.courseId)
            item.put("courseName", course.courseName)
            item.put("description", course.description)
            item.put("linkedBankId", course.linkedBankId ?: JSONObject.NULL)
            item.put("importedAt", course.importedAt)

            val sectionsArray = JSONArray()
            course.sections.forEach { section ->
                val sItem = JSONObject()
                sItem.put("id", section.id)
                sItem.put("chapterTitle", section.chapterTitle)
                sItem.put("sectionTitle", section.sectionTitle)
                sItem.put("order", section.order)
                sItem.put("summary", section.summary)
                sItem.put("content", section.content)
                sItem.put("codeExample", section.codeExample)
                sItem.put("codeExplanation", section.codeExplanation)
                sItem.put("difficulty", section.difficulty)
                sItem.put("questionTag", section.questionTag)
                sItem.put("prerequisites", JSONArray(section.prerequisites))
                sectionsArray.put(sItem)
            }
            item.put("sections", sectionsArray)
            array.put(item)
        }
        return array.toString()
    }

    private fun parseKnowledgeCoursesJson(text: String?): List<KnowledgeCourse> {
        ...
        return runCatching {
            ...
            buildList {
                for (i in 0 until array.length()) {
                    ...
                    val sectionsArray = item.optJSONArray("sections")
                    val sections = if (sectionsArray != null) {
                        buildList {
                            for (j in 0 until sectionsArray.length()) {
                                val sObj = sectionsArray.optJSONObject(j) ?: continue
                                val prereqs = mutableListOf<String>()
                                val prereqArr = sObj.optJSONArray("prerequisites")
                                if (prereqArr != null) {
                                    for (k in 0 until prereqArr.length()) {
                                        prereqs.add(prereqArr.optString(k))
                                    }
                                }
                                add(KnowledgeSection(
                                    id = sObj.optString("id"),
                                    chapterTitle = sObj.optString("chapterTitle"),
                                    sectionTitle = sObj.optString("sectionTitle"),
                                    order = sObj.optInt("order", 0),
                                    summary = sObj.optString("summary"),
                                    content = sObj.optString("content"),
                                    codeExample = sObj.optString("codeExample"),
                                    codeExplanation = sObj.optString("codeExplanation"),
                                    difficulty = sObj.optString("difficulty", "easy"),
                                    prerequisites = prereqs,
                                    questionTag = sObj.optString("questionTag")
                                ))
                            }
                        }
                    } else emptyList()
                    val linkedBankId = if (item.has("linkedBankId") && !item.isNull("linkedBankId")) {
                        item.optString("linkedBankId").ifBlank { null }
                    } else null
    ...
    /**
     * 序列化学习进度（简单 Map 范式）
     * key 格式："${courseId}::${sectionId}"，value 为 SectionProgress
     */
    private fun studyProgressToJson(progress: Map<String, SectionProgress>): String {
        val root = JSONObject()
        progress.forEach { (key, p) ->
            if (key.isBlank()) return@forEach
            val item = JSONObject()
            item.put("studied", p.studied)
            item.put("practiced", p.practiced)
            item.put("correctCount", p.correctCount)
            item.put("totalCount", p.totalCount)
            item.put("lastStudiedAt", p.lastStudiedAt)
            item.put("bestAccuracy", p.bestAccuracy.toDouble())
            item.put("practiceCount", p.practiceCount)
            root.put(key, item)
        }
        return root.toString()
    }

    private fun parseStudyProgressJson(text: String?): Map<String, SectionProgress> {
        if (text.isNullOrBlank()) return emptyMap()
        return runCatching {
            val root = JSONObject(text)
            val result = mutableMapOf<String, SectionProgress>()
            val keys = root.keys()
            while (keys.hasNext()) {
                val key = keys.next()
                if (key.isBlank()) continue
                val item = root.optJSONObject(key) ?: continue
                result[key] = SectionProgress(
                    studied = item.optBoolean("studied", false),
                    practiced = item.optBoolean("practiced", false),
                    correctCount = item.optInt("correctCount", 0),
                    totalCount = item.optInt("totalCount", 0),
                    lastStudiedAt = item.optLong("lastStudiedAt", 0L),
                    bestAccuracy = item.optDouble("bestAccuracy", 0.0).toFloat(),
                    practiceCount = item.optInt("practiceCount", 0)
                )
            }
            result
        }.getOrDefault(emptyMap())
     ...
     * 构造章节进度 key
     */
    fun studyProgressKey(courseId: String, sectionId: String): String = "$courseId::$sectionId"

    /**
     * 获取指定课程-章节的进度
     */
    fun sectionProgress(courseId: String, sectionId: String): SectionProgress {
        return studyProgress[studyProgressKey(courseId, sectionId)] ?: SectionProgress()
    }

    /**
     * 标记章节已学习
     */
    fun markSectionStudied(courseId: String, sectionId: String) {
        val key = studyProgressKey(courseId, sectionId)
        val current = studyProgress[key] ?: SectionProgress()
        if (current.studied && current.lastStudiedAt > 0) return
     ...
     * 记录章节练习结果
     */
    fun recordSectionResult(courseId: String, sectionId: String, correctCount: Int, totalCount: Int) {
        if (totalCount <= 0) return
        val key = studyProgressKey(courseId, sectionId)
        val current = studyProgress[key] ?: SectionProgress()
        val accuracy = correctCount.toFloat() / totalCount
        val bestAccuracy = maxOf(current.bestAccuracy, accuracy)
        studyProgress[key] = current.copy(
            studied = true,
            practiced = true,
     ...
     * 通过 courseId 获取课程
     */
    fun courseById(courseId: String): KnowledgeCourse? {
        return knowledgeCourses.firstOrNull { it.courseId == courseId }
    }

    /**
     * 获取课程的章节列表（按 order 排序）
     */
    fun sectionsForCourse(courseId: String): List<KnowledgeSection> {
        return courseById(courseId)?.sections
            ?.sortedBy { it.order } ?: emptyList()
    }

    /**
     * 获取某章节关联的题目（通过 knowledgePoints 标签筛选）
     */
    fun questionsForSection(courseId: String, sectionId: String): List<Question> {
        ...
        return bank.questions.filter { q ->
            q.knowledgePoints.any { it in tags }
        }
    }
    ...
    data class CourseProgressSummary(
        ...
        val studiedSections: Int,
        val practicedSections: Int,
    ...
    fun courseProgressSummary(courseId: String): CourseProgressSummary {
        val sections = sectionsForCourse(courseId)
        var studied = 0; var practiced = 0; var mastered = 0
        var answered = 0; var correct = 0
        var accSum = 0f; var accCount = 0
        sections.forEach { s ->
            val p = sectionProgress(courseId, s.id)
            if (p.studied) studied++
            if (p.practiced) practiced++
            if (p.masteryLevel == MasteryLevel.MASTERED) mastered++
            if (p.practiced) {
                answered += p.totalCount
                correct += p.correctCount
                accSum += p.bestAccuracy
                accCount++
            }
        }
        val totalQ = questionsForCourseCount(courseId)
        val avg = if (accCount > 0) accSum / accCount else 0f
        return CourseProgressSummary(
            totalSections = sections.size,
            studiedSections = studied,
    ...
    private fun questionsForCourseCount(courseId: String): Int {
        val course = courseById(courseId) ?: return 0
        val bank = course.linkedBankId?.let { id -> banks.firstOrNull { it.id == id } } ?: return 0
    ...
    fun deleteCourse(context: Context, courseId: String) {
        appContext = context.applicationContext
        val course = courseById(courseId) ?: return
        // 清理进度
        val keysToRemove = studyProgress.keys.filter { it.startsWith("$courseId::") }
        keysToRemove.forEach { studyProgress.remove(it) }
        // 移除课程
        knowledgeCourses.removeAll { it.courseId == courseId }
        // 删除关联题库
        course.linkedBankId?.let { bankId ->
            if (banks.any { it.id == bankId }) {
    ...
    /**
     * 导入课程包（核心入口）
     * 解析 course 节点 + questions 数组，建立双向关联
     * @return 导入结果描述（成功/失败原因）
     */
    fun importCoursePackage(context: Context, rawJson: String): String {
        appContext = context.applicationContext
        return runCatching {
            val text = rawJson.trim()
            if (text.isBlank()) return@runCatching "课程包内容为空"

            val root = JSONObject(text)
            val courseObj = root.optJSONObject("course")
                ?: return@runCatching "未检测到课程包格式：缺少 course 节点"

            val courseId = courseObj.optString("courseId").ifBlank {
                return@runCatching "课程包缺少 courseId 字段"
            }
            val courseName = courseObj.optString("courseName").ifBlank { "未命名课程" }
            ...
            if (sectionsArray == null || sectionsArray.length() == 0) {
                return@runCatching "课程包缺少 sections 章节数据"
            }

            // 解析章节
            val sections = buildList {
                for (i in 0 until sectionsArray.length()) {
                    val sObj = sectionsArray.optJSONObject(i) ?: continue
                    ...
                    add(KnowledgeSection(
                        ...
                        difficulty = sObj.optString("difficulty", "easy"),
                        prerequisites = prereqs,
                        questionTag = sObj.optString("questionTag", id)
                    ))
                }
            }
            ...
            val questions = if (questionsArray != null) {
                parseQuestionsArray(questionsArray)
            } else emptyList()

            // 解析题库名
            val bankName = root.optString("bankName").ifBlank { "$courseName 题目" }

            // 重复导入检查：若已存在同 courseId 的课程，先删除旧的
            ...
            if (existing != null) {
                ...
                keysToRemove.forEach { studyProgress.remove(it) }
                // 删除旧题库
                existing.linkedBankId?.let { oldBankId ->
                    if (banks.any { it.id == oldBankId }) {
                        deleteBank(context, oldBankId)
                    }
                }
                knowledgeCourses.removeAll { it.courseId == courseId }
            }
            ...
            // 创建关联题库
            var linkedBankId: String? = null
            if (questions.isNotEmpty()) {
                importBank(context, bankName, questions, DEFAULT_BANK_GROUP_NAME)
                linkedBankId = activeBankId
            }

            // 创建课程对象
            val course = KnowledgeCourse(
                courseId = courseId,
            ...
            knowledgeCourses.add(course)
            persist()

            "成功导入课程「$courseName」：${sections.size} 个章节${if (questions.isNotEmpty()) "、${questions.size} 道题目" else ""}"
        }.getOrElse { e ->
